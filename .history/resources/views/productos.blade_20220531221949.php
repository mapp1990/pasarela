        ...
        @foreach($products as $producto)
            <div class="bg-dark col-6 col-md text-center text-white" style="margin-top: 20px">
                ...
                <div class="bg-light shadow-sm mx-auto" style="width: 80%; border-radius: 21px 21px 0 0;">
                    ...
                    <form method="POST" action="/pasarela/public/verificacion_informacion">
                        @csrf
                        <input type="hidden" name="nombreProduct" id="nombreProduct" value="{{$producto->nombre}}"/>
                        <input type="hidden" name="descripcionProduct" id="descripcionProduct" value="{{$producto->descripcion}}"/>
                        <input type="hidden" name="precioProduct" id="precioProduct" value="{{$producto->precio}}"/>
                        <button type="submit" class="btn btn-info" style="margin-top: 30px; margin-bottom: 30px;">Comprar</button>
                    </form>
                </div>
            </div>
        @endforeach
