...
class PlaceToPayController extends Controller{
    ...
    public function __construct(){
        ...
        $this->endpoint = 'https://test.placetopay.com/redirection/api/notify';
    }
    ...
    public function getAuth(){
		...
		$placetopay = new \Dnetix\Redirection\PlacetoPay([//new Orders([
			"login" => $this->login,
			"tranKey" => base64_encode(sha1($nonce . $seed . $this->scretKey, true)), //$this->scretKey,
			"url" => $this->endpoint,
			"seed" => $seed,
			"nonce" => $nonce,
    ...
    public function createPayRequest(Request $request){
		...
		$dataenvio = [
      	...
      	dd($placetopay->request($dataenvio));
